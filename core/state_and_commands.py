...
async def set_user_image_edit_session(user_id:int, session:dict|None)->None:
    ...
    await user_visual_session.set(user_id, session)

async def get_user_image_edit_session(user_id:int)->dict|None:
    """Получает visual session для пользователя."""
    return await user_visual_session.get(user_id, None)

async def clear_user_image_edit_session(user_id:int)->None:
    """Очищает visual session для пользователя."""
    await user_visual_session.set(user_id, None)
...
async def send_service_notification(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        if not context.args:
            await reply_service_text(update, "Вы не задали сервисное оповещение. Команда должна выглядеть так: /send_notification <сообщение> ==<user id> или /send_notification <сообщение>")
            return
    ...
    if in_admin_list(user):
        ...
        if user_id_str is not None and user_id_str!= "":
            try:
                ...
                await reply_service_text(update,"Вы не задали сервисное оповещение. Команда должна выглядеть так: /send_notification <оповещение> ==<user id> или /send_notification <оповещение>")
                return
