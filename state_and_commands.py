...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await reply_service_text("Контекст беседы был сброшен. Начинаем новую беседу.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await in_user_list(user):
        ...
        return
    await reply_text('Привет! Я бот, интегрированный с ChatGPT. Задайте мне вопрос.')

async def add_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if in_admin_list(user):
        if len(context.args) == 0:
            await reply_service_text("Вы не указали идентификатор пользователя. Команда должна выглядеть так: /add <идентификатор пользователя>")

            return
        elif not context.args[0].isdigit():
            await reply_service_text("Идентификатор должен быть числом.")
            return
        ...
        if new_user_id not in temp_user_ids:
            ...
            await reply_service_text("Пользователь добавлен в список допустимых.")
        else:
            await reply_service_text("Пользователь уже в списке допустимых.")
    else:
        await reply_service_text("У вас нет прав на эту команду.")

async def remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if in_admin_list(user):
        if len(context.args) == 0:
            await reply_service_text("Вы не указали идентификатор пользователя. Команда должна выглядеть так: /remove <идентификатор пользователя>")
        ...
        if new_user_id in temp_user_ids:
            ...
            await reply_service_text("Пользователь удален из списка допустимых.")
        else:
            await reply_service_text("Пользователь не найден в списке допустимых.")
    else:
        await reply_service_text("У вас нет прав на эту команду.")
...
async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if in_admin_list(user):
        temp_user_ids = await get_all()
        await reply_service_text("Список допустимых пользователей: " + str(temp_user_ids))
    else:
        await reply_service_text("У вас нет прав на эту команду.")
...
async def get_last_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if in_admin_list(user):
        ...
        if len(last_sessions)==0:
            await reply_service_text("Список последних сессий пользователей пуст.")
            return
        ...
        await reply_service_text(last_sesssions_str)
    else:
        await reply_service_text("У вас нет прав на эту команду.")
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if in_user_list(user):
        ...
        if version == "":
            await reply_service_text("Нет данных.")
        else:
            await reply_service_text(f"Версия бота: {version}, модель: {model_name}, модель для распознавания голоса: {voice_recognition_model_name}")
    else:
        await reply_service_text("У вас нет прав на эту команду.")
